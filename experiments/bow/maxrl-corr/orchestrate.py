"""
Orchestrator for one MaxRL bag-of-words sweep (single dataset, single seed).
...
Usage:
    uv run python experiments/bow/maxrl-corr/orchestrate.py \\
        --dataset homoskedastic --seed 51 --subtract-baseline True
    uv run python experiments/bow/maxrl-corr/orchestrate.py \\
        --dataset homoskedastic --seed 51 --subtract-baseline True --dry-run
"""
    ...
    *,
    ...
    with log_path.open("ab") as log_fh:
        header = (
            ...
            f"baseline={baseline_mode} ({len(jobs)} jobs) =====\n"
        )
        ...
        for corr, num_rollouts in jobs:
            ...
            tag = (
                ...
                f"rollouts={num_rollouts} baseline={baseline_mode}]"
            )
...
def main(dataset: str, seed: int, subtract_baseline: bool, dry_run: bool) -> None:
    jobs_by_device = {dev: build_jobs(device_id=dev) for dev in (0, 1)}
    baseline_mode = baseline_mode_folder(subtract_baseline=subtract_baseline)

    print(f"dataset={dataset} seed={seed} baseline={baseline_mode}")
