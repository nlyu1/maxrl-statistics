...
`--factorized` is required and has no default — the caller (e.g. sweep.bash)
must supply it explicitly so the variant is never implicit.

Usage:
    uv run python experiments/corpus-regression/rloo/orchestrate.py \\
        --seed 51 --factorized True
    uv run python experiments/corpus-regression/rloo/orchestrate.py \\
        --seed 51 --factorized True --dry-run
"""
...
SINGLE_RUN = repo_root / "experiments" / "corpus-regression" / "rloo" / "single_run.py"
METHOD = "rloo"
    ...
    *,
    ...
    with log_path.open("ab") as log_fh:
        header = (
            ...
            f"factorized={factorized_mode} ({len(jobs)} jobs) =====\n"
        )
...
def main(seed: int, factorized: bool, dry_run: bool) -> None:
    jobs_by_device = {dev: build_jobs(device_id=dev) for dev in (0, 1)}
    factorized_mode = factorized_mode_folder(factorized=factorized)

    print(f"seed={seed} factorized={factorized_mode}")
    ...
    log_dir = artifacts_dir() / METHOD / "logs" / factorized_mode
