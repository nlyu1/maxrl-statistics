...
def _leave_one_out_weight(
    *,
    ...
    Base case p_0^{loo, j} = 1. The loop stops at k = N - 2, so N - k >= 2.
    Not a convex combination: both terms can reach O(N) while their difference
    is in [0, 1], so the recurrence loses digits to cancellation on rollouts
    with likelihood << beta. We run it in float64 (see compute_maxrl_score_weights).
    """
...
def compute_maxrl_score_weights(
    *,
    ...
    Notes:
        Internally runs in float64; the LOO recurrence subtracts two O(N)
        quantities to produce one in [0, 1], and fp32's ~7 digits are
        consumed by the cancellation on low-likelihood rollouts once
        N >~ 64. Output is cast back to the input dtype.
    """
    ...
    with torch.no_grad():
        ...
        tilde_l, a = _scaled_likelihoods_and_complements(
            log_likelihoods=log_likelihoods.double(),
