...
class BagOfWordsDatasetConfig(BaseConfig):
    ...
    num_val_samples: int
    prompt_length: int
    word_assignments: list[str]  # ordered negative → positive
    ...
    def initialize(
        ...
        *,
        ...
        assert n % 2 == 1, "word_assignments must have odd length (neutral center word)"
        half = n // 2
        ...
        mean = sum(word_density[w] * word_values[w] for w in word_density)
        std = (
            sum(word_density[w] * word_values[w] ** 2 for w in word_density) - mean**2
        ) ** 0.5
        return cls(
            snr=snr,
