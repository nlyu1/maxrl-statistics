...
NUM_ROLLOUT_STEPS = 4     # T: "thinking tokens" generated per rollout
...
STUDY_FOLDER = (
    ...
    / f"grpo_G{NUM_ROLLOUT_SAMPLES}_T{NUM_ROLLOUT_STEPS}"
)
...
def sft_step(
    ...
    with torch.autocast(device_type=DEVICE.type, dtype=torch.bfloat16):
        prediction = model(tokens=tokens).squeeze(-1)
...
def grpo_microbatch(
    ...
    with torch.no_grad():
        rewards: Float[Tensor, "B G"] = -(projections.detach() - target_expanded) ** 2
    ...
    projection_loss = F.mse_loss(projections, target_expanded)
    total_loss = (policy_loss + PROJECTION_LOSS_WEIGHT * projection_loss) / GRAD_ACCUM_STEPS
...
def main():
    ...
    data_config, dataset, train_dl, val_dl = setup_data()
    ...
    for step in tqdm(range(NUM_SFT_WARMUP_STEPS), desc="sft warmup"):
        try:
            tokens, target, ground_truth = next(train_iter)
        ...
        with torch.cuda.device(DEVICE):
            loss = sft_step(model, optimizer, tokens, target)

        with torch.no_grad():
            pred = model(tokens=tokens).squeeze(-1).float()
            warmup_corr.tick(x=pred[:, None], y=target.float()[:, None])

    warmup_stats = warmup_corr.get_stats()
    print(f"SFT warmup done. Train corr: {float(warmup_stats.corr.squeeze(0).cpu()):.4f}")
    ...
    print(f"\n=== Phase 2: GRPO training (grad_accum={GRAD_ACCUM_STEPS}) ===")
    model.train()
    ...
    for epoch in range(NUM_GRPO_EPOCHS):
        ...
        optimizer.zero_grad(set_to_none=True)
        pbar = tqdm(train_dl, desc=f"grpo epoch {epoch}")
        for tokens, target, ground_truth in pbar:
            ...
            pbar.set_postfix(
                pol_loss=f"{mb_metrics['policy_loss']:.4f}",
                proj_loss=f"{mb_metrics['projection_loss']:.4f}",
                corr_t=f"{tc:.4f}",
                corr_gt=f"{gc:.4f}",
            )
        ...
        num_microbatches = epoch_policy_loss / max(epoch_policy_loss, 1e-12)  # just count
        total_mb = len(train_dl)
